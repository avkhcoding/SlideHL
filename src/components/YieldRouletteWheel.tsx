...
export interface YieldRouletteSegment {
  id: string
  label: string
  type: 'none' | 'points_multiplier' | 'yield_claim' | 'mystery'
...
const segments: YieldRouletteSegment[] = [
  { id: '1', label: 'Try Again', type: 'none', amount: 0, probability: 30, color: 'hsl(var(--muted))', icon: Repeat },
  { id: '2', label: 'Try Again', type: 'none', amount: 0, probability: 30, color: 'hsl(var(--muted))', icon: Repeat },
  { id: '3', label: 'Points 1.5x', type: 'points_multiplier', amount: 1.5, probability: 15, color: 'hsl(var(--primary))', icon: Sparkles },
  { id: '4', label: 'Points 3x', type: 'points_multiplier', amount: 3, probability: 8, color: 'hsl(var(--accent))', icon: Sparkles },
  { id: '5', label: 'Yield Claim', type: 'yield_claim', amount: 'Daily', probability: 10, color: 'hsl(var(--warning))', icon: TrendingUp },
  { id: '6', label: 'Mystery Prize', type: 'mystery', amount: 'Special', probability: 7, color: 'hsl(242 99% 71%)', icon: Gift }
]

interface YieldRouletteWheelProps {
  weeklyYield: number
  userSpinsToday: number
  maxSpinsPerDay: number
  spinPrice: number
  onSpin: (result: YieldRouletteSegment) => void
  disabled?: boolean
}

export function YieldRouletteWheel({ 
  weeklyYield, 
  userSpinsToday, 
  maxSpinsPerDay, 
  spinPrice, 
  onSpin, 
  disabled = false 
}: YieldRouletteWheelProps) {
  const [isSpinning, setIsSpinning] = useState(false)
  const [lastResult, setLastResult] = useState<YieldRouletteSegment | null>(null)
  ...
  const getRandomSegment = (): YieldRouletteSegment => {
  ...
  return (
    <Card className="bg-gradient-to-br from-card to-card/50 border-warning/20 overflow-hidden">
      <CardContent className="p-6 text-center">
        {/* Title */}
        <div className="mb-6">
          <h3 className="text-2xl font-bold text-card-foreground mb-2">Weekly Yield Roulette</h3>
        ...
        <div className="relative mb-6">
          ...
          <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-2 z-10">
            <div className="w-0 h-0 border-l-[12px] border-r-[12px] border-b-[20px] border-l-transparent border-r-transparent border-b-warning drop-shadow-lg" />
          </div>
        ...
        <div className="space-y-4">
          <Button
            onClick={handleSpin}
            disabled={!canSpin}
            className="w-full bg-warning hover:bg-warning/90 text-warning-foreground py-6 text-lg font-semibold rounded-xl"
            size="lg"
          >
            {isSpinning ? (
              <>
        ...
        {/* Last Result */}
        {lastResult && (
          <div className="mt-6 p-4 bg-gradient-to-r from-warning/10 to-accent/10 rounded-xl border border-warning/20">
